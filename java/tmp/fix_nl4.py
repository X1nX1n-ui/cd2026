...
if old1 in data:
    ...
    changes += 1
    print("1. Input replaced")

# === 2. humanToCron ===
cn_chars = C("一二三四五六七八九十两零半")
cn_map_str = '"一":1,"二":2,"三":3,"四":4,"五":5,"六":6,"七":7,"八":8,"九":9,"十":10,"零":0,"两":2,"半":30'

regex_body = (
    b'            if (/[\\u5206]|min/i.test(t) && !/[\\u5c0f\\u65f6]|[\\u65f6](?!\\u95f4)|[\\u70b9]|[\\u5929]|[\\u65e5]/i.test(t)) {\n'
    b"                return '0 */' + nums[0] + ' * * * ?';\n"
    b"            }\n"
    b'            if (/[\\u5c0f\\u65f6]|[\\u65f6](?!\\u95f4)/i.test(t) && !/[\\u5929]|[\\u65e5]/i.test(t)) {\n'
...
old2 = b"function cronToHuman(cron) {"
new2 = (
    b"function humanToCron(text) {\n"
    b"            if (!text) return '';\n"
    b"            var t = text.trim();\n"
    b"            if (/^[0-9*\\/,?\\-]+(\\s+[0-9*\\/,?\\-]+){4,5}$/.test(t)) return t;\n"
    b"            var cn = {" + cn_map_str.encode("utf-8") + b"};\n"
    b"            t = t.replace(/[" + cn_chars + b"]+/g, function(m) { return cn[m] || m; });\n"
...
# === 3. Preset ===
old3 = b"updateScheduleHint(root, cronValue);"
new3 = (
...
if old3 in data:
    ...
    changes += 1
    print("3. Preset handler updated")

# === 4. Save ===
old4 = b'var cronExpr = root.querySelector("#cronCustomInput").value.trim();'
new4 = (
